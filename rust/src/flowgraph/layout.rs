...
/// Registers a custom flow graph layout, this allows you to customize the layout of flow graphs.
...
pub trait FlowGraphLayout: Sized + Sync + Send + 'static {
    /// Places the provided `nodes` inside the `graph`, returning `true` if successful.
